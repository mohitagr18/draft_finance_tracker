...
async def run_parsing_agent():
    ...
    assistant = AssistantAgent(
        ...
        system_message=(
            ...
            "1. Parses `statement` into a JSON object with keys:\n"
            "   - 'transactions_by_cardholder': a dictionary where each key is a cardholder name "
            "     and the value is a list of {date, description, amount}.\n"
            "   - 'summary': contains 'bank_name', 'total_transactions','total_amount','previous_balance','payments','credits','purchases','cash_advances','fees','interest','new_balance','available_credit_limit'.\n"
            "2. Ensure amounts are numbers (floats) and NOT zero.\n"
            "3. Prints **only** the JSON via `print(json.dumps(parsed, ensure_ascii=False))`.\n"
